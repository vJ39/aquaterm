...
| `W` | クジラを1匹確実に追加(通常個体よりずば抜けて大きいネタ枠。捕食されず、繁殖もしない) |
| `M` | ピラニア専用の肉餌を投下(空腹なら確実に食いつく。満腹の間は無視する。自動モードには組み込まれない) |
| `D` | タコつぼを再配置 |
| `P` | 藻・水草を再配置 |
| `,` | 設定画面を開く(効果音/オーバーレイ/自動モード等を一覧切替。生み出す魚の種類・餌の量・カニ表示のトグルも含む) |
...
ピラニアやタコは他の魚を捕食しますが、魚は藻や岩に隠れて身を守れます。タコは追われると墨を吐いて逃げ、無敵になれるスターも低頻度で出現します。
...
- 状態保存先(`~/.config/aquaterm/state.json`)は`$HOME`環境変数を直接参照する自前実装で、OS固有のAPIには依存していない

</details>

## Save data

`~/.config/aquaterm/state.json` に各魚・卵・餌・薬・藻・岩・タコつぼ等の状態と経過時間を保存します。`q` 終了時に保存し、起動時に読み込んで再開します(ファイルが無ければ初期状態で開始)。血飛沫・墨・カメオ生物など一瞬〜数十秒で消える演出や、自動モード/自動魚補充/昼夜連動のON/OFFは保存対象外です。
