...
`~/.config/aquaterm/state.json` に各魚・卵・餌・薬・藻・岩・タコつぼ等の状態と経過時間を保存します。`q` 終了時に保存し、起動時に読み込んで再開します(ファイルが無ければ初期状態で開始)。血飛沫・墨・カメオ生物・水流の筋など一瞬〜数十秒で消える演出や、自動モード/自動魚補充/昼夜連動のON/OFFは保存対象外です。
