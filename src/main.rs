...
fn sfx_enabled_for(ctl: &Ctl, ev: sim::SfxEvent) -> bool {
    match ev {
        ...
        | sim::SfxEvent::WhaleExplosion => ctl.sfx_on,
    }
}
...
fn render_tank(
    ...
    for f in &sim.fish {
        ...
        // 水底に沈み切った死骸(corpse_uses_lying_sprite)は、それより優先して
        // 専用の横倒れスプライトを使う(死骸は方向転換しないため優先順位の
        // 競合は起きない)。
...
mod tests {
    ...
    fn events_without_a_dedicated_toggle_follow_the_master_sfx_on() {
        ...
        assert!(!sfx_enabled_for(&ctl, sim::SfxEvent::WhaleExplosion));
