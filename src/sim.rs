...
// --- 個体差(新規・2026/07/14) ---
// 実機フィードバック(「各魚、タコ、ピラニアみんな腹の減り方、食ったときの腹の満たされ方は
// 個体差があるものとする」「大きくなるサイズも同様に個体差があるものとし、寿命も個体差が
// あるものとする」「たまに大食いのやつがいても面白い」)対応。種による違いではなく、
...
// 大きな藻の株・岩塊に描き直した上でこの距離も広げた(旧6.0→9.0)。さらに実機
// フィードバック(「まだ小さい。魚のドット絵より明らかに大きいサイズに」)を受けて
// 藻をさらに大きく描き直したため、この距離も合わせて広げた(旧9.0)。
pub const ALGAE_HIDE_RADIUS: f64 = 14.0;
...
// 実機フィードバック(「まだ薄い。もっと濃く広範囲に真っ黒に」)を受けてさらに拡大した(旧28.0)。
...
// 実機フィードバック(「食欲もっと旺盛に。3匹食べないと満腹にならないようにして」)対応。
...
// 実機フィードバック(「捕食するやつ(ピラニア・タコ)ばかりだと魚がいなくなる」)対応。
...
impl Simulation {
        ...
        if self.plants.is_empty() {
            for _ in 0..PLANT_COUNT {
                ...
                self.plants.push(Plant {
                    x,
                    y: sand_top,
                    // 実機フィードバック(「藻を魚が隠れられるくらい大きく」→まだ小さい
                    // との再指摘で再拡大)対応: 旧サイズ(3.0〜7.0→6.0〜11.0)から、
                    // 魚のドット絵より明らかに大きい・完全に覆えるサイズまで拡大した。
                    height: self.rng.range(12.0, 20.0),
    ...
    pub fn reposition_dens(&mut self, pix_w: usize, pix_h: usize) {
        // 実機フィードバック(「再配置と同時にタコつぼの数を現在のタコの数に整理して
        // ほしい。タコより多いタコつぼは削除・少なければ追加」)対応: タコつぼの数を
    ...
    pub fn reposition_plants(&mut self, pix_w: usize, pix_h: usize) {
        ...
        for _ in 0..count {
            ...
            self.plants.push(Plant {
                x,
                y: sand_top,
                // ensure_decorative_entities側と同じ大きさに揃える(旧3.0〜7.0のまま
                // 直し忘れていた不整合を修正)。
                height: self.rng.range(12.0, 20.0),
                phase: self.rng.range(0.0, std::f64::consts::TAU),
            });
    ...
    // 既存の自動モード(aキー・update_auto_care)とは別のトグルにする(実機フィードバック
    // 「捕食するやつばかりだと魚がいなくなる」対応)。通常魚(捕食対象になる種。
    ...
    fn update_movement(&mut self, dt: f64, w: f64, sand_top: f64) {
        ...
        for i in 0..self.fish.len() {
            ...
            // 実機フィードバック(バグ報告「青い魚がタコに向かって突進して捕食された」)対応:
            ...
            // 実機フィードバック(「大きく成長したピラニアが底面に堆積した肉餌を食べられない」)
            // 対応: 肉餌への接近中(nearest_meat)も、生きた獲物の追跡中(is_chasing)と
            // 同様にマージンを基本値へ戻す。そうしないと、大きく育った個体ほどサイズ基準の
            // マージンで壁際・水底に十分近づけず、着地した肉餌との間に詰め切れない
            // 隙間が残ってしまう。
    ...
    fn update_meat(&mut self, dt: f64, sand_top: f64, pix_w: usize) {
        ...
        for f in &mut self.fish {
            ...
            let mut best_dist = f64::INFINITY;
            let mut best_mi = None;
            // 実機フィードバック(「大きく成長したピラニアが底面に堆積した肉餌を
            // 食べられない」)対応: 判定距離を生きた獲物と同じPIRANHA_STRIKE_RADIUS
    ...
    fn update_predation(&mut self, dt: f64) {
        ...
        'outer: for (i, f) in self.fish.iter().enumerate() {
            ...
            ) in snapshot.iter().enumerate()
            {
                ...
                // 中心・口どちらか近い方を使う(実機フィードバック「大きく成長した
                // ピラニアが肉餌を食べられない」で判明した同種の問題への予防的対応:
...
mod tests {
    ...
    fn large_piranha_can_still_eat_meat_landed_in_a_bottom_corner() {
        // 実機フィードバック(「大きく成長したピラニアが底面に堆積した肉餌を食べられ
        // ない。壁際とか、端っこ関連」)の回帰テスト。最大まで育った(拡大サイズの)
    ...
    fn auto_replenish_adds_a_fish_when_common_species_are_scarce() {
        // 実機フィードバック(「捕食するやつ(ピラニア・タコ)ばかりだと魚がいなくなる」)
    ...
    fn reposition_dens_trims_excess_dens_down_to_the_current_octopus_count() {
        // 実機フィードバック(「タコつぼの数を現在のタコの数に整理してほしい。
        // タコより多いタコつぼは削除」)対応: 自然にタコが減って(老衰等)空きタコつぼが
