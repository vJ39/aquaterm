...
// スケールとは別枠のベース倍率)。1.0のベースに加算されるため、見た目は通常成魚の約2.75倍になる。
// (2026/07/16: ドットが荒いとの指摘を受けてfish.rs側のクジラパターンを縦横約1.3倍の
// 高解像度に描き直した際、旧値2.5のままだと解像度upぶんだけ画面上のサイズも
// 大きくなってしまうため、ほぼ同じ表示サイズを保つよう1.75へ調整した)
pub const WHALE_BASE_SCALE_BONUS: f64 = 1.75;
...
// (単一の細い水草・6.0)から段階的に拡大してきた(6.0→9.0→14.0→20.0)。それでもまだ
// 小さいとの再指摘を受けて、藻の高さ(Plant.height)・岩の拡大描画(ROCK_SCALE)を
// さらに一回り大きくしたのに合わせて再度広げた(20.0→28.0)。
pub const ALGAE_HIDE_RADIUS: f64 = 28.0;
...
// 血の匂いの発生源にごく近い距離まで来た後の挙動: この半径の内側では
// 「中心へ一直線に向かう」吸引をそのまま使わない。理由は、出血元は
// 生きた獲物と違って動かない一点なので、強い吸引(PIRANHA_BLOOD_SCENT_PULL)を
// 距離に関係なく効かせ続けると、中心を通り過ぎるたびに向きがほぼ180度反転し、
// 中心のごく近くで細かく左右に振動し続けてしまう(実機で確認された「プルプル」
// する不自然な動きの原因)。この半径の内側では吸引を弱めつつ接線方向の
// 周回成分を混ぜ、中心付近をなめらかにうろつく動きに変える。
pub const PIRANHA_BLOOD_SCENT_LOITER_RADIUS: f64 = 16.0;
// 周回(うろつき)成分の強さ。値が大きいほど中心の周りを大きく回り込む。
// 通常の狩りの吸引(PIRANHA_HUNT_PULL)よりはっきり弱く、「落ち着いてうろつく」
// 程度の緩やかさにとどめる。
pub const PIRANHA_BLOOD_SCENT_LOITER_ORBIT_STRENGTH: f64 = 90.0;
// 周回の向きが切り替わる速さ。ピラニアの逃走ジグザグ(ZIGZAG_FREQ=3.0)より
// 大幅にゆっくりにし、細かい振動に見えないようにする。
pub const PIRANHA_BLOOD_SCENT_LOITER_FREQ: f64 = 0.5;
...
// 設定画面表示用: 餌の量レベル(0..=4)の表示ラベル。表示言語に応じて出し分けるため
// (以前はconst配列だったが、実行時に言語が切り替わる文言はconstにできない)関数にしてある。
pub fn feed_amount_label(level: usize) -> &'static str {
    const JA: [&str; FEED_AMOUNT_LEVELS] = ["少なめ", "ふつう", "多め", "かなり多め", "どっぱー(MAX)"];
    const EN: [&str; FEED_AMOUNT_LEVELS] = ["Light", "Normal", "Heavy", "Extra heavy", "MAX (dump it all)"];
    let table = if i18n::is_en() { &EN } else { &JA };
    table.get(level).copied().unwrap_or(table[FEED_AMOUNT_DEFAULT])
}
...
// --- 方向転換(左右反転)時の体ひねりフレックス演出 ---
// facing_right が反転した瞬間、真横向きから反対の真横向きへ即座に切り替わるのではなく、
// 実際の魚が体を捻って向きを変える際の見た目に近づけるため、ごく一瞬だけ頭側を
// 体の中心へ寄せた「ひねり」を経てから、新しい向きの通常の横向きプロファイルに戻す
// (fish.rs の Fish::turn_flex_intensity が経過時間から強さを判定し、Fish::display_sprite
// がいま表示中のプロファイル自身を変形する。専用の別ドット絵は持たないため、直前・
// 直後の通常フレームと同じ絵から連続的に変化して見える。この演出はFish::display_sprite
// 側だけに閉じ、Fish::sprite/render_scale等の判定用の値には影響させない)。
...
impl Simulation {
        ...
        if self.plants.is_empty() {
            for _ in 0..PLANT_COUNT {
                ...
                self.plants.push(Plant {
                    ...
                    // との再指摘を受けて段階的に拡大してきた
                    // (旧3.0〜7.0→6.0〜11.0→12.0〜20.0→20.0〜32.0)。それでもまだ小さいとの
                    // 再指摘を受けて、もう一段大きく描き直した。
                    height: self.rng.range(32.0, 50.0),
    ...
    pub fn reposition_dens(&mut self, pix_w: usize, pix_h: usize) {
        ...
        if self.dens.is_empty() && octo_count == 0 {
            self.set_message(i18n::t("タコつぼがありません", "No octopus dens"));
            return;
        }
        ...
        self.set_message(i18n::t("タコつぼを再配置しました", "Repositioned the octopus dens"));
    }
    ...
    pub fn reposition_plants(&mut self, pix_w: usize, pix_h: usize) {
        if self.plants.is_empty() {
            self.set_message(i18n::t("藻・水草がありません", "No plants"));
            return;
        }
        ...
        self.set_message(i18n::t("藻・水草を再配置しました", "Repositioned the plants"));
    }
    ...
    pub fn reset(&mut self, pix_w: usize, pix_h: usize) {
        ...
        self.set_message(i18n::t("水槽をリセットしました", "Tank reset"));
    }
    ...
    pub fn feed(&mut self, cursor_x: f64, pix_w: usize) {
        ...
        if self.feed_amount == FEED_AMOUNT_LEVELS - 1 {
            self.set_message(i18n::t("どっぱー！餌を大量投入した", "SPLASH! Dumped in a huge amount of food"));
        }
    }
    ...
    pub fn add_octopus(&mut self, pix_w: usize, pix_h: usize) {
        if self.fish.len() >= ADD_FISH_MANUAL_CAP {
            self.set_message(i18n::t("これ以上は孵化でしか増えません", "No more room; only hatching can add more now"));
            return;
        }
        if self.living_count() >= self.effective_cap(pix_w, pix_h) {
            self.set_message(i18n::t("水槽が満員です", "The tank is full"));
            return;
        }
        ...
        self.fish.push(octo);
        self.set_message(i18n::t("タコを1匹投入しました", "Added an octopus"));
    }
    ...
    pub fn debug_force_courtship_proximity(&mut self, pix_w: usize, pix_h: usize) {
        ...
        if mated {
            self.set_message(i18n::t(
                "つがいを即座に交尾させた(デバッグ)",
                "Forced a pair to mate instantly (debug)",
            ));
        } else {
            self.set_message(i18n::t(
                "産卵可能な同種ペアがいない(デバッグ)",
                "No same-species pair ready to breed (debug)",
            ));
        }
    }
    ...
    pub fn debug_starve_all(&mut self) {
        ...
        self.set_message(i18n::t("全員を空腹にした(デバッグ)", "Made everyone hungry (debug)"));
    }
    ...
    pub fn debug_grow_all_to_adult(&mut self) {
        ...
        let msg = if i18n::is_en() {
            format!("Grew {count} fry to adult (debug)")
        } else {
            format!("稚魚{count}匹を成魚にした(デバッグ)")
        };
    ...
    pub fn debug_toggle_pollution(&mut self) {
        ...
        self.set_message(i18n::t(
            if self.pollution >= POLLUTION_MAX / 2.0 {
                "水質を最悪にした(デバッグ)"
            } else {
                "水質を綺麗にした(デバッグ)"
            },
            if self.pollution >= POLLUTION_MAX / 2.0 {
                "Set water quality to worst (debug)"
            } else {
                "Set water quality to clean (debug)"
            },
        ));
    }
    ...
    pub fn debug_kill_random_fish(&mut self) {
        ...
        if alive_indices.is_empty() {
            self.set_message(i18n::t("生きている個体がいない(デバッグ)", "No living fish (debug)"));
            return;
        }
        ...
        let msg = if i18n::is_en() {
            format!("Forcibly killed {} (debug)", species_name(sp))
        } else {
            format!("{}を強制的に死亡させた(デバッグ)", species_name(sp))
        };
        self.set_message(msg);
    }
    ...
    pub fn debug_age_random_fish_near_death(&mut self) {
        ...
        let msg = if i18n::is_en() {
            format!("Set {}'s remaining lifespan to 10s (debug)", species_name(sp))
        } else {
            format!("{}の寿命を残り10秒にした(デバッグ)", species_name(sp))
        };
        self.set_message(msg);
    }
    ...
    pub fn debug_spawn_star(&mut self, cursor_x: f64, cursor_y: f64, pix_w: usize, pix_h: usize) {
        ...
        self.set_message(i18n::t("スターを投入した(デバッグ)", "Spawned a star (debug)"));
    }
    ...
    pub fn debug_explode_whale(&mut self) {
        let Some(idx) = self.fish.iter().position(|f| f.species == Species::Whale) else {
            self.set_message(i18n::t("クジラがいない(デバッグ)", "No whale (debug)"));
            return;
        };
    ...
    // また、あるペア(i, j)がiの番で判定に失敗した場合でもpairedはfalseのままなので、
    // ループが後でjの番に進むと同じペア(i, j)へ再度独立した確率判定が行われてしまい、
    // 1tick中に同一ペアが最大2回判定される(実質的な成立確率がほぼ倍になる)バグが
    // あった。これを防ぐため、このtick内で既に判定を試みた(失敗した)ペアをtriedに
    // 記録し、同じペアへの再判定をスキップする。
    fn update_breeding_pairs(&mut self, dt: f64, spawn_eggs: &mut Vec<(f64, f64, Species, bool)>) {
        ...
        for &(i, fx, fy, sp) in &ready {
            ...
            let msg = if i18n::is_en() {
                format!("A pair of {} mated", species_name(sp))
            } else {
                format!("{}のつがいが交尾した", species_name(sp))
            };
            self.set_message(msg);
        }
    }
    ...
    fn lay_egg_cluster(&mut self, px: f64, sp: Species, sand_top: f64, pix_w: f64, mated: bool) -> String {
        ...
        if i18n::is_en() {
            format!("{} laid eggs", species_name(sp))
        } else {
        ...
        for f in &mut self.fish {
            ...
            if !f.hidden && f.ink_cooldown <= 0.0 {
                ...
                if threatened || fish_approached {
                    ...
                    self.message =
                        Some(i18n::t("タコが墨を吐いて逃げた!", "The octopus inked and fled!").to_string());
                    self.message_ttl = 4.0;
                }
            }
        }
        ...
        for (oi, ox, oy, bite_count, immunity_timer) in octopuses {
            if immunity_timer <= 0.0 {
                ...
                if biter_in_range && self.rng.next_f64() < OCTOPUS_BITE_CHANCE_PER_SEC * dt {
                    ...
                    let msg = if i18n::is_en() {
                        if bite_number >= OCTOPUS_BITES_TO_DIE {
                            "The octopus was bitten by a fish and died…".to_string()
                        } else if bite_number == 1 {
                            "The octopus was bitten by a fish…".to_string()
                        } else {
                            "The octopus was bitten again and is weakening…".to_string()
                        }
                    } else if bite_number >= OCTOPUS_BITES_TO_DIE {
    ...
    fn update_movement(&mut self, dt: f64, w: f64, sand_top: f64) {
        ...
        for i in 0..self.fish.len() {
            ...
            if let Some(((bx, by), dist)) = blood_scent_target {
                let d = dist.max(0.001);
                let scent_dir_x = (bx - fx) / d;
                let scent_dir_y = (by - fy) / d;
                if dist < PIRANHA_BLOOD_SCENT_LOITER_RADIUS {
                    // 出血元のごく近くまで来た後は、動かない一点へ全力で向かい続ける
                    // 吸引をそのまま使わない(生きた獲物と違って中心が逃げてくれない
                    // ため、そのまま使うと通り過ぎるたびに向きがほぼ反転して細かく
                    // 振動してしまう)。中心へ近いほど直進の吸引を弱め、代わりに
                    // 進行方向に対して垂直な、ゆっくり向きを変える周回成分を混ぜて、
                    // 中心付近をなめらかにうろつく動きにする。
                    let approach_frac = dist / PIRANHA_BLOOD_SCENT_LOITER_RADIUS;
                    let pull = PIRANHA_BLOOD_SCENT_PULL * approach_frac;
                    ax += scent_dir_x * pull;
                    ay += scent_dir_y * pull;
                    let phase = self.elapsed * PIRANHA_BLOOD_SCENT_LOITER_FREQ + i as f64 * 2.3;
                    let orbit = phase.sin() * PIRANHA_BLOOD_SCENT_LOITER_ORBIT_STRENGTH;
                    ax += -scent_dir_y * orbit;
                    ay += scent_dir_x * orbit;
                } else {
                    ax += scent_dir_x * PIRANHA_BLOOD_SCENT_PULL;
                    ay += scent_dir_y * PIRANHA_BLOOD_SCENT_PULL;
                }
            }
    ...
    fn update_medicine(&mut self, dt: f64, sand_top: f64, pix_w: usize) {
        ...
        if cured {
            self.set_message(i18n::t("薬で病気が治った", "The medicine cured the sickness"));
    ...
    fn update_meat(&mut self, dt: f64, sand_top: f64, pix_w: usize) {
        ...
        if bitten {
            self.set_message(i18n::t(
                "ピラニアが肉餌を食べて満腹になった",
                "A piranha ate the meat and is full",
            ));
            self.sound_events.push(SfxEvent::Predation);
        }
    ...
    fn update_stars(&mut self, dt: f64) {
        ...
        if let (Some(fi), Some(si)) = (best_fi, best_si) {
            ...
            self.set_message(i18n::t(
                "スターを取得!無敵状態になった",
                "Picked up a star! Invincible now",
            ));
    ...
    fn update_biology(&mut self, dt: f64, dyn_cap: usize, cap: usize, w: f64, sand_top: f64) {
        ...
        for f in &mut self.fish {
                ...
                && f.has_mated
            {
                ...
                messages.push(if i18n::is_en() {
                    format!("{} reached old age and laid a final clutch of eggs", species_name(f.species))
                } else {
                    format!("{}が老齢に差し掛かり、最後の卵を産んだ", species_name(f.species))
                });
            }
            ...
            if !f.sick {
                    ...
                    && self.rng.next_f64() < DISEASE_CHANCE_PER_SEC * stress_mult * pollution_sick_mult * dt
                {
                    ...
                    f.sick_timer = 0.0;
                    messages.push(if i18n::is_en() {
                        format!("{} got sick… press [m] for medicine", species_name(f.species))
                    } else {
                        format!("{}が病気になった…[m]で薬を", species_name(f.species))
                    });
            ...
            if !f.sick {
                ...
                if f.stage == Stage::Fry && f.well_fed_timer >= GROW_TIME {
                    ...
                    f.well_fed_timer = 0.0;
                    messages.push(if i18n::is_en() {
                        format!("{} grew into an adult", species_name(f.species))
                    } else {
                        format!("{}が成魚に育った", species_name(f.species))
                    });
                }
                    ...
                    && f.size_timer >= SIZE_GROW_TIME
                {
                    ...
                    f.size_timer = 0.0;
                    messages.push(if i18n::is_en() {
                        format!("{} grew even bigger", species_name(f.species))
                    } else {
                        format!("{}がさらに大きく育った", species_name(f.species))
                    });
                }
            ...
            if f.starve_timer >= STARVE_DEATH_TIME || (f.sick && f.sick_timer >= SICK_DEATH_TIME) {
                f.dead = true;
                f.dead_timer = 0.0;
                deaths.push(if i18n::is_en() {
                    format!("{} died…", species_name(f.species))
                } else {
                    format!("{}が力尽きた…", species_name(f.species))
                });
            } else if f.age >= LIFESPAN_DEATH_AGE * f.lifespan_mult {
                f.dead = true;
                f.dead_timer = 0.0;
                deaths.push(if i18n::is_en() {
                    format!("{} died of old age…", species_name(f.species))
                } else {
                    format!("{}が老衰で力尽きた…", species_name(f.species))
                });
                ...
                self.sound_events.push(SfxEvent::Predation);
                deaths.push(i18n::t(
                    "タコが浄化剤の効きすぎで血を吐いて力尽きた…",
                    "The octopus bled out from purifier overdose…",
                ).to_string());
                ...
                self.sound_events.push(SfxEvent::Predation);
                deaths.push(if i18n::is_en() {
                    format!("A {} fry bled out from the purifier…", species_name(f.species))
                } else {
                    format!("{}の稚魚が浄化剤で大量出血して力尽きた…", species_name(f.species))
                });
            }
        }
        ...
        if hatched_msg {
            messages.push(i18n::t("卵が孵化した", "An egg hatched").to_string());
        }
        if hatch_failed_msg {
            messages.push(
                i18n::t(
                    "水質が悪く、卵が孵化できなかった…",
                    "Poor water quality kept an egg from hatching…",
                )
                .to_string(),
            );
        }
        ...
        if let Some(m) = deaths.into_iter().last() {
            ...
            if self.message.is_none() {
                let msg = if i18n::is_en() {
                    format!("{} is weakening…", species_name(f.species))
                } else {
                    format!("{}が弱っている…", species_name(f.species))
                };
                self.set_message(msg);
            }
        }
    ...
    fn update_predation(&mut self, dt: f64) {
        ...
        if let (Some(si), Some(pi)) = (predator_index, prey_index) {
            ...
            if predator_species == Species::Piranha {
                ...
                let msg = if i18n::is_en() {
                    if piranha_bite_number >= PIRANHA_BITES_TO_KILL {
                        format!("{} was attacked by a piranha and died…", species_name(prey_species))
                    } else if piranha_bite_number == 1 {
                        format!("{} was bitten hard by a piranha…", species_name(prey_species))
                    } else {
                        format!("{} was bitten by a piranha and is near death…", species_name(prey_species))
                    }
                } else if piranha_bite_number >= PIRANHA_BITES_TO_KILL {
                ...
                let msg = if i18n::is_en() {
                    format!("{} was eaten…", species_name(prey_species))
                } else {
                    format!("{}が食べられた…", species_name(prey_species))
                };
                self.set_message(msg);
            }
        }
    ...
    fn trigger_whale_explosion(&mut self, whale_idx: usize) {
        ...
        let msg = if i18n::is_en() {
            format!("The whale exploded… {victims} fish in the tank were caught in the blast")
        } else {
            format!("クジラが大爆発した…水槽内の{victims}匹が犠牲になった")
        };
        self.set_message(msg);
    }
...
mod tests {
    ...
    #[test]
    fn piranha_loiters_near_a_blood_scent_instead_of_vibrating_through_its_center() {
        // 実機フィードバック回帰テスト: 血の匂い(捕食跡)の発生源のごく近くに
        // いるピラニアが、中心へ強く引き寄せられて左右に細かく振動する
        // (「プルプルする」不自然な動き)のではなく、周辺を落ち着いてうろつく
        // 動きになっているはず。中心付近で高頻度に速度の向きが反転していない
        // ことを、うろつき半径内での短時間での速度符号反転回数で確認する。
        let mut sim = Simulation::new(Rng::new(321));
        let mut piranha = Fish::new(Species::Piranha, Stage::Adult, 100.0, 100.0);
        piranha.hunger = MAX_HUNGER; // 満腹(狩りのchase_targetが常にNoneのままにする)
        piranha.predation_cooldown = PIRANHA_HUNT_COOLDOWN; // クールダウン中でもある
        sim.fish.push(piranha);
        // 血の匂いを、うろつき半径(PIRANHA_BLOOD_SCENT_LOITER_RADIUS=16)より
        // 内側のごく近くに置く(振動が起きるのはまさにこの近距離)。
        sim.blood_scents.push(BloodScent {
            x: 105.0,
            y: 100.0,
            life: BLOOD_SCENT_LIFETIME,
        ...
        let dt = 0.05;
        let mut prev_vx: Option<f64> = None;
        let mut sign_flips = 0usize;
        for _ in 0..80 {
            // 4秒間観察する
            sim.update(dt, 400, 400);
            let vx = sim.fish[0].vx;
            // ごく小さいランダムウォーク由来の符号変化までは数えず、はっきり
            // 意味のある速度が出ているときの反転だけを数える。
            if vx.abs() > 1.0 {
                if let Some(p) = prev_vx {
                    if (p > 0.0) != (vx > 0.0) {
                        sign_flips += 1;
                    }
                }
        ...
        assert!(
            sign_flips <= 8,
            "血の匂いの中心付近で速度の向きが高頻度に反転(振動)しているはず: sign_flips={sign_flips}"
        );
    }
    ...
    fn breeding_pair_within_mate_radius_eventually_mates_and_resets_both_timers() {
        ...
        // 確率的なイベントを待つテストのため、他の箇所の変更でtickごとの乱数消費数が
        // 変わってもタイミングがずれにくいよう、十分大きめの試行回数にしてある。
        // 元は40000だったが、このseed(9)では乱数列の巡り合わせ上、成立まで実測で
        // 約47480tick(約59254回の確率判定)かかることが判明し、40000では届かず
        // 単体実行時に確実に失敗していた(既存バグ)。500000あれば見積もりの
        // 期待値(約12500回の判定で成立)に対して十分な安全マージンがある。
        for _ in 0..500_000 {
            for f in &mut sim.fish {
                f.hunger = MAX_HUNGER;
                f.well_fed_timer = BREED_READY_TIME + 5.0;
    ...
    #[test]
    fn same_pair_is_not_rolled_twice_in_the_same_tick() {
        // 回帰テスト: 以前のバグでは、ペア(i, j)がiの番で確率判定に失敗しても
        // pairedはfalseのままなので、ループが後でjの番に進むと同じペア(i, j)へ
        // 再度独立した判定が行われてしまい、1tickで最大2回判定されていた。
        //
        // このテストでは、xorshift64乱数の最初の2つのf64出力(draw1, draw2)が
        // draw1 > 0.001 > draw2 となる既知のシード(29307901)を選び、
        // dt=1.25(BREED_CHANCE_PER_SEC(0.0008) * dt = 0.001)で呼び出す。
        // draw1(iの番の判定)は0.001以上なので失敗するが、もし修正前のように
        // jの番で同じペアへ再判定(draw2を消費)してしまうと、draw2(0.001未満)で
        // 成立してしまう。修正後は同一ペアへの再判定自体が起きないはずなので、
        // このtickでは交尾は成立しない。
        let seed = 29307901;
        let dt = 1.25;
        assert!(
            (BREED_CHANCE_PER_SEC * dt - 0.001).abs() < 1e-9,
            "このテストはBREED_CHANCE_PER_SEC*dt=0.001になる前提で乱数列を選んでいるため、\
             定数を変更した場合はシード/dtの再選定が必要"
        );
        ...
        let mut spawn_eggs = Vec::new();
        sim.update_breeding_pairs(dt, &mut spawn_eggs);

        assert!(
            spawn_eggs.is_empty(),
            "1回目の判定(iの番)は失敗する乱数列を選んでいるため、このtickでは交尾は\
             成立しないはず(成立していたら同一ペアへの二重判定が再発している疑い)"
        );
        assert!(
            !sim.fish[0].has_mated && !sim.fish[1].has_mated,
            "交尾が成立していない以上、has_matedは両方ともfalseのままのはず"
        );
    }

    #[test]
    fn breeding_success_rate_matches_single_roll_probability_not_double() {
        // 回帰テスト: 大量試行で成立率を統計的に検証する。修正前は1tickにつき
        // 同一ペアが最大2回判定されうるため、実質的な成立確率が理論値
        // (BREED_CHANCE_PER_SEC * dt)のほぼ2倍になっていた。修正後は1tickにつき
        // 1回だけの判定になるはずなので、大量試行(trials回)での成立回数は
        // 理論値(trials * p)に近い値になり、2倍の値には遠く及ばないはず。
        let mut sim = Simulation::new(Rng::new(123));
        sim.fish
            .push(Fish::new(Species::Guppy, Stage::Adult, 40.0, 30.0));
        sim.fish
            .push(Fish::new(Species::Guppy, Stage::Adult, 41.0, 30.0));
        ...
        let dt = 0.1;
        let p = BREED_CHANCE_PER_SEC * dt; // 1ペア・1tickあたりの意図した成立確率
        let trials = 2_000_000;
        let mut mate_count = 0usize;
        for _ in 0..trials {
            // 常に「産卵準備完了」状態を維持し、毎tick独立した判定機会を作る
            for f in &mut sim.fish {
                f.hunger = MAX_HUNGER;
                f.well_fed_timer = BREED_READY_TIME + 5.0;
                f.has_mated = false;
            ...
            let mut spawn_eggs = Vec::new();
            sim.update_breeding_pairs(dt, &mut spawn_eggs);
            if !spawn_eggs.is_empty() {
                mate_count += 1;
            }
        }

        let expected = trials as f64 * p;
        // 二重判定バグが再発していれば成立回数は理論値のほぼ2倍に張り付くはずなので、
        // その中間より十分下に上限を置く。下限は判定自体が機能していないケース
        // (成立回数が異常に少ない)を検出するためのもの。
        let upper_bound = expected * 1.5;
        let lower_bound = expected * 0.5;
        assert!(
            (mate_count as f64) < upper_bound,
            "成立回数{}が理論値(trials*p={:.1})の1.5倍を超えている(二重判定バグの再発疑い)",
            mate_count,
            expected
        );
        assert!(
            (mate_count as f64) > lower_bound,
            "成立回数{}が理論値(trials*p={:.1})の半分未満(判定自体が機能していない疑い)",
            mate_count,
            expected
        );
    }
    ...
    fn is_hidden_in_cover_detects_proximity_to_plants_and_rocks() {
        ...
        sim.rocks.push(Rock { x: 200.0, y: 10.0 });

        assert!(sim.is_hidden_in_cover(10.0, 10.0), "藻の真上は隠れているはず");
        assert!(sim.is_hidden_in_cover(200.0, 10.0), "岩の真上は隠れているはず");
        assert!(
            !sim.is_hidden_in_cover(105.0, 10.0),
    ...
    #[test]
    fn hunting_piranha_does_not_vibrate_when_cornered_prey_cannot_flee_further() {
        // 調査(新規): 血の匂いの追跡と違い、生きた獲物への通常の狩り吸引
        // (chase_target/hunt_pull、距離減衰なしで一定の強さがかかり続ける点は
        // 血の匂いの修正前と同型)は、獲物が壁際・角に追い詰められて実質
        // 動けない状況でも「中心を通り過ぎては反転して戻る」細かい左右の振動
        // (プルプル)を起こさないことの回帰テスト。
        //
        // 検証手法は血の匂いの回帰テスト(piranha_loiters_near_a_blood_scent_
        // instead_of_vibrating_through_its_center)と同じ: 短時間での速度符号
        // 反転回数を数える。cornered_prey_is_eventually_caught_even_at_the_
        // tank_cornerと同じ角の配置を使い、獲物は逃走を続けるが壁で実質
        // 動けない状態にする。
        //
        // 血の匂いと違い生きた獲物では、以下の構造的な違いにより振動が
        // 起きないと考えられる:
        //   1. 血の匂いは接触しても消費されず、いつまでも同じ強さで
        //      引き寄せ続けるため、中心を通り過ぎるたびに反転が繰り返される。
        //      一方、生きた獲物は捕食判定(PIRANHA_STRIKE_RADIUS)に入った
        //      その場のtickで即座に捕食が成立し(update_movement→
        //      update_predationの順で同tick内に処理される)、追跡吸引
        //      (chase_target)自体がクールダウンでその後しばらく消える。
        //      つまり「中心を通り過ぎる」状況が発生する前に追跡が打ち切られる。
        //   2. 壁際の角では、獲物・捕食者とも追跡・逃走中は同じ基本マージンに
        //      そろえられるため、獲物が捕食者の反対側へ回り込まれてしまう
        //      ような幾何学的な反転も起きにくい。
        let (w, h) = (100, 40);
        let mut sim = Simulation::new(Rng::new(613));
        sim.fish.push(Fish::new(Species::Neon, Stage::Adult, 3.0, 8.0)); // 左上の角付近(壁で実質逃げ場なし)
        let mut piranha = Fish::new(Species::Piranha, Stage::Adult, 15.0, 12.0);
        piranha.hunger = PIRANHA_HUNT_HUNGER_THRESHOLD - 10.0; // 捕食モード
        piranha.predation_cooldown = 0.0;
        sim.fish.push(piranha);
        ...
        let mut prev_vx: Option<f64> = None;
        let mut sign_flips = 0usize;
        let mut reached_close_range = false;
        let mut struck = false;
        for _ in 0..500 {
            // 30秒(500*0.05)。cornered_prey_is_eventually_caught_even_at_the_
            // tank_cornerと違い、ここではクールダウンを毎tick強制リセットしない
            // (自然に1回だけ噛みつくまでの接近フェーズをそのまま観察したいため)。
            sim.fish[1].hunger = PIRANHA_HUNT_HUNGER_THRESHOLD - 10.0; // 捕食条件を維持
            sim.update(dt, w, h);
            let dist = ((sim.fish[1].x - sim.fish[0].x).powi(2) + (sim.fish[1].y - sim.fish[0].y).powi(2)).sqrt();
            if dist < PIRANHA_HUNT_RADIUS {
                reached_close_range = true;
            }
            // 噛みつき(=クールダウンが立つ)が発生したら、そこで観察を打ち切る
            // (以降は血の匂いの追跡・クールダウンに切り替わり、別の既存テストの
            // 対象になるため)。
            if sim.fish[1].predation_cooldown > 0.0 || sim.fish[0].dead {
                struck = true;
                break;
            }
            let vx = sim.fish[1].vx;
            // ごく小さいランダムウォーク由来の符号変化までは数えず、はっきり
            // 意味のある速度が出ているときの反転だけを数える。
            if vx.abs() > 1.0 {
                if let Some(p) = prev_vx {
        ...
        assert!(
            reached_close_range,
            "この検証が意味を持つには、追跡中に検知範囲(PIRANHA_HUNT_RADIUS)の近距離まで実際に近づく必要がある"
        );
        assert!(
            struck,
            "壁際に追い詰めた獲物は、振動して固まるのではなく最終的に捕食(噛みつき)が成立するはず"
        );
        assert!(
            sign_flips <= 8,
            "壁際で逃げ場を失った獲物を追う間、速度の向きが高頻度に反転(振動)しているはず: sign_flips={sign_flips}"
        );
    }
}
