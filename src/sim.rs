    ...
    fn update_movement(&mut self, dt: f64, w: f64, sand_top: f64) {
        ...
        for i in 0..self.fish.len() {
            ...
            let wall_push_suppressed =
                is_chasing || is_fleeing_predator || flee_timer > 0.0 || ink_escape_timer > 0.0;
    ...
    fn update_meat(&mut self, dt: f64, sand_top: f64, pix_w: usize) {
        ...
        for f in &mut self.fish {
            ...
            for (mi, mt) in self.meat.iter().enumerate() {
                ...
                let d = ((mt.x - f.x).powi(2) + (mt.y - f.y).powi(2)).sqrt();
                if d < EAT_RADIUS && d < best_dist {
                    best_dist = d;
                    best_mi = Some(mi);
                }
            ...
            if let Some(mi) = best_mi {
                // 空腹の間だけ食いつき、満腹まで回復する。旺盛な食欲ロジックの
                // カウンタもリセットする(肉餌を与えて狩りを止められる救済ツールにする)。
                f.hunger = MAX_HUNGER;
