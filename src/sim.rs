...
// 浮力が指数関数的に減衰する時定数(秒)。この値だと死亡からおよそ5分
// (300秒=DEAD_FLOAT_TIMEの目安)で浮力が重力を下回り、沈み始める計算になる。
pub const CORPSE_BUOYANCY_DECAY_TAU: f64 = 433.0;
// 本番ロジックでは閾値として直接は使わない(浮力減衰の連続計算に委ねている)が、
// おおよその沈み始めタイミングの目安としてテスト・ドキュメントで参照する。
#[allow(dead_code)]
pub const DEAD_FLOAT_TIME: f64 = 300.0;
