...
// 相対的にどんどん綺麗になっていく。
pub const POLLUTION_MAX: f64 = 100.0;
pub const POLLUTION_PER_LANDED_FOOD: f64 = 0.03; // 水底に堆積した餌1個あたり/秒
pub const POLLUTION_PER_LANDED_MEDICINE: f64 = 0.03; // 水底に堆積した薬1個あたり/秒
pub const POLLUTION_PER_LANDED_MEAT: f64 = 0.05; // 水底に堆積した肉餌1個あたり/秒(食べ残りやすいぶん重め)
// 病気・死亡個体は「急激に悪化」の要望どおり、堆積物より一段強いレートにする。
pub const POLLUTION_PER_SICK_FISH: f64 = 0.3;
pub const POLLUTION_PER_DEAD_FISH: f64 = 0.6;
pub const POLLUTION_NATURAL_DECAY: f64 = 0.15; // 常時かかる自然浄化(悪化要因が無ければ改善していく)
// 病気の発症確率(DISEASE_CHANCE_PER_SEC)にかかる乗数の上限。水質最悪(POLLUTION_MAX)で
// 発症確率が最大でこの倍になる(「水質が悪いと生物が死にやすくなる」を病気ルート経由で表現)。
...
impl Simulation {
        ...
        if self.plants.is_empty() {
            for _ in 0..PLANT_COUNT {
                ...
                self.plants.push(Plant {
                    ...
                    // との再指摘を受けて再拡大した。旧サイズ(3.0〜7.0→6.0〜11.0)から、
    ...
    fn update_biology(&mut self, dt: f64, cap: usize, w: f64, sand_top: f64) {
        ...
        let vanished_octopus_dens: Vec<(f64, f64)> = self
            .fish
            .iter()
            .filter(|f| f.species == Species::Octopus && f.dead && f.dead_timer >= DEAD_FLOAT_TIME)
            .map(|f| (f.den_x, f.den_y))
            .collect();
...
mod tests {
    ...
    fn vanishing_octopus_removes_its_own_den() {
        ...
        sim.fish[0].dead_timer = DEAD_FLOAT_TIME; // 浮遊時間を経過済みにする
